...
function parseParam(request, options) {
    "use strict";
    var name    = options.nameOf.param,
...
function parseQuery(request, options) {
    "use strict";
    var name    = options.nameOf.query,
...
function parseCookie(request, options) {
    "use strict";
    var name    = options.nameOf.cookie,
        key     = options.nameOf.cookieKey,
...
function parseHeader(request, options) {
    "use strict";
    var name    = options.nameOf.header,
...
function getRequestedLocales(request, options) {
    ...
    for (i = 0; i < options.order.length; i = i + 1) {
        ...
        //    };
        //}
        requestedLocales = orderParameters[options.order[i]](request, options);
        if (requestedLocales ) {
            return {
                source: options.order[i],
...
function determineLocale(request, options) {
    ...
    if (requestedLocales.source.indexOf('params') === 0 || options.throw404) {
        throw new Error('Requested localization "' + requestedLocales.locales.join(', ') + '" is not available.');
    }
...
function processRequest(options, request, reply) {
    ...
    if (typeof options.setter === 'string' && options.setter !== '' && options.createAccessorsIfNotExists && !lodash.has(request, options.setter)) {
        lodash.set(request, options.setter, function(newLocale) {
            locale = newLocale;
            return locale;
        });
    }
...
exports.register = function(server, options, next) {
    ...
    server.expose('getLocale', function getLocale(request, reply) {
        try {
            return options.getter ? lodash.get(request, options.getter)() : determineLocale(request, options);
        } catch(err) {
            return null;
        }
    ...
    server.ext(options.onEvent, processRequest.bind(undefined, options));
